...
async function initMap() {
  ...
  // Handle map clicks when enabled
  map.on('click', (e) => {
    if (mapClickEnabled && selectedItem) {
      currentPin = { lat: e.latlng.lat, lng: e.latlng.lng };
      const note = document.getElementById('locationNote');
      const enableBtn = document.getElementById('enableMapSelect');
      const confirmBtn = document.getElementById('confirmLocation');
      
      if (note) {
        note.textContent = `Location selected! Click "Confirm This Location" to proceed.`;
        note.classList.add('show');
        note.style.color = '#caac00';
      }
      
      // Hide enable button, show confirm button
      if (enableBtn) {
      ...
      // Add temporary marker to show selected location (scaled to current zoom)
      if (window.tempMarker) {
  ...
  if (enableMapBtn) {
    enableMapBtn.addEventListener('click', () => {
      ...
      enableMapBtn.innerHTML = '<i class="fas fa-hand-pointer"></i><span>Now click on the map to set location</span>';
      
      // Hide confirm button if visible
      if (confirmBtn) {
        confirmBtn.style.display = 'none';
      }
      
      // Make modal backdrop semi-transparent and allow map clicks
      const backdrop = document.querySelector('.modal-backdrop');
      if (backdrop) {
        backdrop.classList.add('map-selection-active');
        backdrop.style.zIndex = '1040';
      }
      
      // Make map container clickable but keep modal visible
      ...
      if (mapEl) {
        ...
        mapEl.style.cursor = 'crosshair'; // Show it's clickable
      }
      
      // Make modal content area allow clicks to pass through to map
      const modalContent = document.querySelector('.modal-content');
      if (modalContent) {
        modalContent.style.pointerEvents = 'auto'; // Keep modal interactive
      }
      
      const note = document.getElementById('locationNote');
      if (note) {
        note.textContent = 'Click anywhere on the map to set the location';
        note.classList.add('show');
        note.style.color = '#caac00';
      }
    });
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener('click', () => {
      ...
      if (isFinite(lat) && isFinite(lng)) {
        ...
        // Update temp marker to show it's confirmed (scaled to current zoom)
        if (window.tempMarker) {
          map.removeLayer(window.tempMarker);
        }
        const currentZoom = map.getZoom();
        const confirmSize = Math.max(24, Math.min(40, 36 + (currentZoom - 17) * -4));
        const confirmFontSize = Math.max(14, Math.min(20, 18 + (currentZoom - 17) * -2));
        window.tempMarker = L.marker([lat, lng], {
          icon: L.divIcon({
            className: 'custom-div-icon',
            html: `<div style="display:flex;align-items:center;justify-content:center;width:${confirmSize}px;height:${confirmSize}px;border-radius:50%;background:#16a34a;color:#fff;border:2px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,.3);"><i class="fas fa-check" style="font-size:${confirmFontSize}px;"></i></div>`,
            iconSize: [confirmSize, confirmSize],
            iconAnchor: [confirmSize / 2, confirmSize]
          })
        }).addTo(map);
        ...
        const backdrop = document.querySelector('.modal-backdrop');
        if (backdrop) {
          backdrop.classList.remove('map-selection-active');
